    ...
    return (
        <div className="bg-backgroundTheme hidden lg:flex flex-col justify-between h-screen border-r-[1px] border-secondaryC py-2 px-4">
            <div className="flex flex-col">
                <div onClick={() => setSelectedSidebar(1)} className={`p-3 space-y-4 cursor-pointer ${selectedSidebar === 1 ? "bg-secondaryCHover" : "bg-secondaryC"} hover:bg-black/10 rounded-t-xl`}>
                    <MessageSquareHeart color={selectedSidebar === 1 ? "#991b1b" : themeMode === "dark" ? "#ffffff" : "#000000"} />
                </div>
                <div onClick={() => setSelectedSidebar(2)} className={`p-3 space-y-4 cursor-pointer ${selectedSidebar === 2 ? "bg-secondaryCHover" : "bg-secondaryC"} hover:bg-black/10 rounded-b-xl`}>
                    <BookUser color={selectedSidebar === 2 ? "#991b1b" : themeMode === "dark" ? "#ffffff" : "#000000"} />
                </div>
            </div>
            <div onClick={handleSignOut} className={`p-3 space-y-4 cursor-pointer bg-secondaryC hover:bg-secondaryCHover rounded-xl flex justify-center items-center`}>
