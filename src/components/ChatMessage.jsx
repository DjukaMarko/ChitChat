    ...
    return (
        <div className={`flex flex-col ${side === 1 ? "items-end" : "items-start"} mt-1`}>
            {isDifference(m, text[index === text.length - 1 ? index : index + 1], text.length, index) && (
                <div className={`w-full flex justify-center border-t-[1px] my-2 border-black/[.03]`}>
                    <p className={`text-xs py-3`}>{compareTimestamps(m, text[index === text.length - 1 ? index : index + 1])}</p>
                </div>
            )}
            {side === 2 && (text[index + 1]?.sentBy !== otherMember?.userId && <img className="w-9 rounded-full mt-4 ml-2" src={otherMember?.photoUrl} />)}
            <div className={`${(side === 1 ? (isMessageSending && index === 0 ? "bg-red-400" : "bg-red-800 hover:bg-red-700") : "bg-black/10 hover:bg-black/20")} ${side === 1 ? "mx-6" : "mx-10"}  py-2 px-4 rounded-xl relative max-w-[70%] break-words`}>
