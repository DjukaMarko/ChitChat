...
export const SearchInput = ({ searchInput, setSearchInput, handleSearchSubmit }) => {
    ...
    return (
        <div className="group">
            <div className="py-2 flex space-x-2 items-center px-5 rounded-full bg-[#f7f7f7] group-hover:bg-[#f0f0f0] font-light outline-0">
                <img src={search} className="w-[16px] h-[16px]" />
                <input
                    value={searchInput}
                    onChange={(e) => setSearchInput(e.target.value)}
                    className="text-sm outline-0 border-0 w-full bg-[#f7f7f7] font-[400] placeholder-[#9e9e9e] group-hover:bg-[#f0f0f0]"
                    type="text"
